...
# Map of curated TMDB poster metadata for standard CinePrime catalog
...
class Command(BaseCommand):
    help = "Synchronizes existing database movie records with official TMDB poster artwork and backdrops."

    def add_arguments(self, parser):
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force re-sync poster data even if poster_path is already set.',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f"Starting TMDB poster sync for {movies.count()} movies..."))

        for movie in movies:
            title_lower = movie.title.lower().strip()

            if not force and movie.poster_path and not movie.poster_path.endswith('placeholder.jpg'):
                duplicate_exists = Movie.objects.filter(poster_path=movie.poster_path).exclude(pk=movie.pk).exists()
                if not duplicate_exists:
                    self.stdout.write(f"  [SKIP] '{movie.title}' already has valid poster_path: {movie.poster_path}")
                    skipped_count += 1
                    continue

            # Check curated catalog first
            catalog_item = OFFICIAL_TMDB_CATALOG.get(title_lower)
            if catalog_item:
                movie.tmdb_id = catalog_item['tmdb_id']
                movie.poster_path = catalog_item['poster_path']
                movie.backdrop_path = catalog_item['backdrop_path']
                movie.tmdb_poster_url = f"https://image.tmdb.org/t/p/w500{catalog_item['poster_path']}"
                movie.tmdb_backdrop_url = f"https://image.tmdb.org/t/p/w1280{catalog_item['backdrop_path']}"
                movie.save()
                synced_count += 1
                self.stdout.write(self.style.SUCCESS(f"  [SYNCED CATALOG] '{movie.title}' -> {movie.poster_path}"))
                continue

            # Query live TMDB API
            year = movie.release_date.year if movie.release_date else None
            tmdb_data = search_tmdb_movie(movie.title, year)
            if tmdb_data and tmdb_data.get('poster_path'):
                ...
                movie.save()
                synced_count += 1
                self.stdout.write(self.style.SUCCESS(f"  [SYNCED API] '{movie.title}' -> {movie.poster_path}"))
            else:
                self.stdout.write(self.style.WARNING(f"  [NOT FOUND] No TMDB poster match for '{movie.title}'"))
                skipped_count += 1

        self.stdout.write(self.style.SUCCESS(f"\nTMDB Poster Sync Finished: {synced_count} updated, {skipped_count} skipped."))
